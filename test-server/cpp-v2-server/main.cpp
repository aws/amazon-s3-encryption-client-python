/*
 * S3 Encryption Test Server - C++ V2
 * 
...
void set_client(const std::string &client_id, std::shared_ptr<S3EncryptionClientV2> client)
{
  ...
  while (client_cache_secret.size() > MAX_CACHED_CLIENTS) {
    ...
    if (evict_it != client_cache_secret.end()) {
      fprintf(stderr, "[CPP-V2] [CACHE-EVICT] Evicting client %s (cache size was %zu)\n",
              lru_client_id.c_str(), client_cache_secret.size());
      client_cache_secret.erase(evict_it);
    }
  }
  
  fprintf(stderr, "[CPP-V2] [CACHE-ADD] Added client %s (cache size now %zu)\n",
...
MHD_Result handle_create_client(struct MHD_Connection *connection,
  ...
  try {
    ...
    // Each client gets a large connection pool since we cannot share HTTP clients
    ...
    fprintf(stderr, "[CPP-V2] handle_create_client exception: %s\n", e.what());
...
void fill_context(Aws::Map<Aws::String, Aws::String> &map,
  ...
  if (metadata.empty()) {
    fprintf(stderr, "[CPP-V2] [DEBUG] fill_context: metadata is empty\n");
    return;
  }

  fprintf(stderr, "[CPP-V2] [DEBUG] fill_context: raw metadata='%s' (length=%zu)\n", 
  ...
  while (pos < current.length()) {
    ...
    fprintf(stderr, "[CPP-V2] [DEBUG] fill_context: parsed pair #%d: key='%s', value='%s'\n",
  ...
  fprintf(stderr, "[CPP-V2] [DEBUG] fill_context: completed, parsed %d pairs into map\n", pair_count);
}
...
MHD_Result handle_get_object(struct MHD_Connection *connection,
  ...
  fprintf(stderr, "[CPP-V2] [DEBUG] GetObject START: thread=%lu, bucket=%s, key=%s, client_id=%s, metadata_length=%zu, range=%s\n", 
  ...
  if (!client) {
    fprintf(stderr, "[CPP-V2] GetObject error: Client not found for client_id=%s\n", client_id.c_str());
  ...
  try {
    ...
    if (!range.empty()) {
      ...
      fprintf(stderr, "[CPP-V2] [DEBUG] GetObject: Setting range=%s\n", range.c_str());
    }
    ...
    fprintf(stderr, "[CPP-V2] [DEBUG] GetObject: encryption context map size=%zu\n", kmsContextMap.size());
    for (const auto& pair : kmsContextMap) {
      fprintf(stderr, "[CPP-V2] [DEBUG] GetObject: context['%s']='%s'\n", 
              pair.first.c_str(), pair.second.c_str());
    }
    
    fprintf(stderr, "[CPP-V2] [DEBUG] GetObject: calling client->GetObject() for key=%s\n", key.c_str());
    
    // Keep outcome alive to ensure stream remains valid
    auto outcome = client->GetObject(request, kmsContextMap);

    fprintf(stderr, "[CPP-V2] [DEBUG] GetObject: client->GetObject() returned for key=%s\n", key.c_str());
    ...
    if (outcome.IsSuccess()) {
      ...
      if (content.empty() && stream.fail()) {
        fprintf(stderr, "[CPP-V2] GetObject error: Failed to read stream for bucket=%s, key=%s\n", 
                bucket.c_str(), key.c_str());
        auto msg = make_error("Failed to read response stream", 500);
        return send_response(connection, 500, msg);
      }
      
      fprintf(stderr, "[CPP-V2] GetObject success: bucket=%s, key=%s, size=%zu bytes\n", 
      ...
      fprintf(stderr, "[CPP-V2] [DEBUG] GetObject FAILED: thread=%lu, key=%s\n", 
              (unsigned long)std::hash<std::thread::id>{}(thread_id), key.c_str());
      fprintf(stderr, "[CPP-V2] [DEBUG] GetObject error details:\n");
      fprintf(stderr, "[CPP-V2] [DEBUG]   - Message: %s\n", error.GetMessage().c_str());
      fprintf(stderr, "[CPP-V2] [DEBUG]   - ExceptionName: %s\n", error.GetExceptionName().c_str());
      fprintf(stderr, "[CPP-V2] [DEBUG]   - ResponseCode: %d\n", (int)error.GetResponseCode());
      fprintf(stderr, "[CPP-V2] [DEBUG]   - ShouldRetry: %s\n", error.ShouldRetry() ? "true" : "false");
      
      auto msg = make_error(outcome.GetError().GetMessage(), 500);
      fprintf(stderr, "[CPP-V2] GetObject AWS error: %s\n", msg.c_str());
      return send_response(connection, 500, msg);
    }
  } catch (const std::exception &e) {
    fprintf(stderr, "[CPP-V2] [DEBUG] GetObject EXCEPTION: thread=%lu, key=%s, what=%s\n",
            (unsigned long)std::hash<std::thread::id>{}(thread_id), key.c_str(), e.what());
    auto msg = make_error(e.what(), 500);
    return send_response(connection, 500, msg);
  } catch (...) {
    fprintf(stderr, "[CPP-V2] [DEBUG] GetObject UNKNOWN EXCEPTION: thread=%lu, key=%s\n",
            (unsigned long)std::hash<std::thread::id>{}(thread_id), key.c_str());
...
MHD_Result handle_put_object(struct MHD_Connection *connection,
                             ...
                             std::string body,
                             std::string metadata) {
  fprintf(stderr, "[CPP-V2] PutObject request: bucket=%s, key=%s, client_id=%s, body_size=%zu\n", 
          bucket.c_str(), key.c_str(), client_id.c_str(), body.length());
  
  auto client = get_client(client_id);
  if (!client) {
    fprintf(stderr, "[CPP-V2] PutObject error: Client not found for client_id=%s\n", client_id.c_str());
    return send_response(connection, 404, "{\"error\":\"Client not found\"}");
  }
  ...
  try {
    ...
    if (outcome.IsSuccess()) {
      fprintf(stderr, "[CPP-V2] PutObject success: bucket=%s, key=%s\n", bucket.c_str(), key.c_str());
      json response = {{"bucket", bucket}, {"key", key}};
      return send_response(connection, 200, response.dump());
    } else {
      auto msg = make_error(outcome.GetError().GetMessage(), 500);
      fprintf(stderr, "[CPP-V2] PutObject AWS error: %s\n", msg.c_str());
      return send_response(connection, 500, msg);
    }
  } catch (const std::exception &e) {
    fprintf(stderr, "[CPP-V2] PutObject exception: %s\n", e.what());
    auto msg = make_error(e.what(), 500);
    return send_response(connection, 500, msg);
  }
}
...
void request_completed(void *cls, struct MHD_Connection *connection,
  ...
  fprintf(stderr, "[CPP-V2] request_completed called, reason=%s, con_cls=%p\n", 
          reason, *con_cls);
...
MHD_Result request_handler(void *cls, struct MHD_Connection *connection,
  ...
  try {
    ...
    if (*con_cls == nullptr) {
      fprintf(stderr, "[CPP-V2] REQUEST START: method=%s, url=%s, version=%s, con_cls=NULL, upload_data_size=%zu\n",
  ...
  if (*con_cls == nullptr) {
    ...
    *con_cls = new std::string();
    fprintf(stderr, "[CPP-V2] REQUEST INIT: allocated new request context for %s %s\n", method, url);
    return MHD_YES;
  }
  
  // LOG: Subsequent calls
  if (is_push && *upload_data_size > 0) {
    fprintf(stderr, "[CPP-V2] REQUEST DATA: %s %s receiving %zu bytes\n", method, url, *upload_data_size);
  } else if (*upload_data_size == 0) {
    fprintf(stderr, "[CPP-V2] REQUEST COMPLETE: %s %s ready for processing\n", method, url);
  }
  ...
  fprintf(stderr, "[CPP-V2] PROCESSING: %s %s\n", method, url);

  // Handle client creation endpoint
  if (is_push && url_str == "/client") {
    fprintf(stderr, "[CPP-V2] Handling /client endpoint\n");
    std::string *body = static_cast<std::string*>(*con_cls);
    MHD_Result result = handle_create_client(connection, *body);
    fprintf(stderr, "[CPP-V2] /client handler returned: %d\n", result);
    return result;
  }

  // Handle object operations
  if (url_str.find("/object/") == 0) {
    fprintf(stderr, "[CPP-V2] Handling /object/ endpoint\n");
    ...
    if (slash_pos != std::string::npos) {
      ...
      if (method_str == "GET") {
        fprintf(stderr, "[CPP-V2] Dispatching to handle_get_object\n");
        ...
        fprintf(stderr, "[CPP-V2] handle_get_object returned: %d\n", result);
        return result;
      } else if (method_str == "PUT") {
        fprintf(stderr, "[CPP-V2] Dispatching to handle_put_object\n");
        std::string *body = static_cast<std::string *>(*con_cls);
        MHD_Result result = handle_put_object(connection, bucket, key, client_id, *body, metadata);
        fprintf(stderr, "[CPP-V2] handle_put_object returned: %d\n", result);
        return result;
      } else {
        fprintf(stderr, "[CPP-V2] Method not allowed: %s\n", method);
    ...
    fprintf(stderr, "[CPP-V2] ERROR: Unrecognized endpoint: %s %s\n", method, url);
    return send_response(connection, 404,
                         "{\"error\":\"Not idea what is happening\"}");
  } catch (const std::exception &e) {
    fprintf(stderr, "[CPP-V2] FATAL: Unhandled exception in request_handler: %s (method=%s, url=%s)\n", 
            e.what(), method, url);
    ...
    try {
      ...
      fprintf(stderr, "[CPP-V2] FATAL: Failed to send error response\n");
      return MHD_NO;
    }
  } catch (...) {
    fprintf(stderr, "[CPP-V2] FATAL: Unknown exception in request_handler (method=%s, url=%s)\n", 
            method, url);
    // Try to send error response, but connection might already be broken
    try {
      return send_response(connection, 500, 
...
void log_mhd_error(void* cls, const char* fmt, va_list ap) {
  fprintf(stderr, "[CPP-V2] [MHD-ERROR] ");
...
MHD_Result notify_connection(void *cls,
  ...
  if (toe == MHD_CONNECTION_NOTIFY_STARTED) {
    fprintf(stderr, "[CPP-V2] [MHD-CONNECT] New connection started\n");
  } else if (toe == MHD_CONNECTION_NOTIFY_CLOSED) {
    fprintf(stderr, "[CPP-V2] [MHD-DISCONNECT] Connection closed\n");
  }
  return MHD_YES;
}
...
int main() {
  ...
  if (num_cores == 0) {
    num_cores = 4;
    fprintf(stderr, "[CPP-V2] [WARNING] CPU core detection failed, defaulting to %u cores\n", num_cores);
  }
  ...
  if (!daemon) {
    fprintf(stderr, "[CPP-V2] Failed to start server on port %d\n", port);
